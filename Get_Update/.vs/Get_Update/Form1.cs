...
namespace Get_Update
{
    public partial class Form1 : Form
    {
        ...
        public Form1()
        {
            ...
            Application.OpenForms["Form1"].Close(); //폼 프로그램 종료 
        }
        public void Get_Files_List()
        {
            ...
            uri = new Uri(SR.ToString()+ "/List.ini");//서버리소스 경로 및 이름
            ...
            {
                {
                    try
                    {
                        GetPrivateProfileString("Files", count.ToString(), "", F_NM, 255, Current_Path + @"\List.ini"); //ini에서 파일갯수가져오기
            ...
            while (cnt < e_cnt)
              {
                try
                  {
                    save_name = @""+TR.ToString() + F_List[cnt].Replace("/", @"\"); //저장경로 지정
                    uri = new Uri(SR.ToString() + F_List[cnt]);   //서버리소스 경로 및 이름
                    MessageBox.Show("원: "+uri.ToString() +"    목:"+save_name);
